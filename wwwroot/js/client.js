...
HTMLElement.prototype.make = function (...types) {
  return types.reduce(
    (lastElem, elemType) =>
      lastElem.appendChild(document.createElement(elemType)),
    this
  );
};
...
function createContent() {
  ...
  button.addEventListener("click", async () => {
    const product = {};
    columns.forEach(
      (col) => (product[col] = document.getElementById(col)?.value)
    );
...
function createTableContents(products) {
  ...
  products.forEach((p) => {
    ...
    const button = row.appendChild(
      document.createElement("td").appendChild(document.createElement("button"))
    );
    button.classList.add("btn", "btn-sm", "btn-danger");
    button.textContent = "Delete";
    button.addEventListener(
      "click",
      async () => await network.deleteProduct(p.id, populateTable, showError)
    );
  });
}
